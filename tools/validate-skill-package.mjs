...
assert(
    pkg.name === "sonar-manage-findings-skill",
    "package name must be sonar-manage-findings-skill"
);
assert(
    pkg.repository?.url ===
        "git+https://github.com/Nick2bad4u/SonarCloud-Skill.git",
    "repository.url must exactly match the GitHub repository for npm trusted publishing"
);
assert(
    skill.path === "skills/sonar-manage-findings",
