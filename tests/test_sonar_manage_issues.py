...
def test_fetch_issue_and_hotspot_payloads(
    ...
    assert sonar_manage_issues.fetch_issues(
        context=context,
        issue_statuses="OPEN",
        page=1,
        page_size=50,
...
def test_issue_mutations_support_dry_run_and_refresh(
    ...
    assert sonar_manage_issues.add_issue_comment(
        context=context,
        issue_key="ISSUE-1",
        text="reviewed",
        dry_run=True,
    )["result"]["endpoint"] == "/api/issues/add_comment"
    assert sonar_manage_issues.assign_issue(
        context=context,
        issue_key="ISSUE-1",
        assignee="nick",
        dry_run=False,
    ...
    assert sonar_manage_issues.transition_issues(
        context=context,
        issue_keys=["ISSUE-1"],
        transition="resolve",
        comment="fixed",
        dry_run=True,
    )["results"][0]["endpoint"] == "/api/issues/do_transition"
...
def test_hotspot_helpers_and_review(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert sonar_manage_issues.hotspot_keys_from_items(
        [{"key": "HOTSPOT-1"}, {"missing": True}, "bad"]
    ) == ["HOTSPOT-1"]
    assert sonar_manage_issues.fetch_hotspot_details(
        context=context,
        hotspots=[{"key": "HOTSPOT-1"}],
    ) == [{"status": "REVIEWED", "line": 10, "message": "message"}]
    assert sonar_manage_issues.review_hotspots(
        context=context,
